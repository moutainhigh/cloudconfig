...
$(function(){
    $.ajax({
        url: 'http://comment.51cto.com/index.php?do=index&m=splendidCmt',
        type: 'get',
        data: {num: 4},
        dataType: 'jsonp',
        jsonp: 'callback',
...
$(function(){
     ...
     */
    $.ajax({
        url: 'http://www.51cto.com/php/article_ajax.php',
        type: 'get',
...
$(function(){
    ...
    if (praise){
        if (praise.indexOf(artid) >= 0) {
            $(".dzdz").css("background","url(http://static4.51cto.com/51cto/cms/2016/images/dzdz.jpg) no-repeat 8px -70px");
        }
    }
});
...
$('a.zhan').click(function() {
    ...
    $.get('http://www.51cto.com/php/article_ajax.php',{type: 'praise', id: artid},function(result){
        if (result.code == 1) {
