...
public class RoleController extends BaseController {
    ...
    public ResponseDbCenter selectRoles(HttpServletRequest req, HttpServletResponse rsp,
        ...
        if ("1".equals(loginUserMap.get("roleId"))){  //如果是超级管理员则加载超级管理员自己公司所有的角色,111111表示注册模块的角色ID
            contentStr=" and   ( r.pcCompanyId='"+loginUserMap.get("pcCompanyId")+"' or r.id='111111' ) ";
    ...
    public ResponseDbCenter deleteRolesByIds(HttpServletRequest req, HttpServletResponse rsp,
        ...
        try {
            ...
            if (!"1".equals("roleId")){//如果不是超级管理员。则需要判断要删除的角色是否是公司的管理员角色
                ...
                for (int i = 0; i <roleList.size(); i++) {
                    Map<String,Object> role=roleList.get(i);
                     if (1==(Integer)role.get("isAdmin")||"111111".equals(role.get("id"))){  //111111 id的角色是模板角色
            ...
            roleService.deleteRolesByIds(idList);

            userService.deleteUserRolesByRoles(idList);
            /**
             * 删除角色下的权限
             */
            for (int i = 0; i < iids.length; i++) {
