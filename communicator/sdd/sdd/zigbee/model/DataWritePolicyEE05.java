 ...
 * 查询锁状态信息
 * */
public class DataWritePolicyEE05  implements DataWritePolicy{

	@Override
	public void writeData(ChannelHandlerContext ctx, Data data)
			throws Exception {
 	      Data0xEE0x05 data0xEE0x05=(Data0xEE0x05) data;

		ByteBuf byteBuf = Unpooled.buffer(3+data0xEE0x05.getLength()+1);
		byteBuf.writeByte(data0xEE0x05.getHeader());
		byteBuf.writeByte(data0xEE0x05.getCmd());
		byteBuf.writeByte(data0xEE0x05.getLength());
		
		byteBuf.writeBytes(Tool.hexStringtoBytes(data0xEE0x05.getLockId()));   		
		
		
		byte[] bytes=new byte[3+data0xEE0x05.getLength()];
		byteBuf.getBytes(0, bytes);
		int calCrc=CRC8.crc8(bytes, bytes.length);
		data0xEE0x05.setCrc(calCrc);
		byteBuf.writeByte(data0xEE0x05.getCrc());
		 String headerCmdLockId="ee"+data0xEE0x05.getCmd()+data0xEE0x05.getLockId();
			try {
