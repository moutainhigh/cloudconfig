 ...
 * 
 */
public class ActualTimeBean extends Data {
	
	public static final Logger LOG = Logger.getLogger(ActualTimeBean.class);
	...
	public long getCrcFromSum() {
		...
		if(multiOpenCard != null){
			...
			for (int i = 0; i < multiOpenCard.length; i++) {
				m1 = (multiOpenCard[i] & 0xFF0000) >> 16;
				m2 = (multiOpenCard[i] & 0x00FF00) >> 8;
				m3 = (multiOpenCard[i] & 0x0000FF);
				multiOpenCardSum += (m1 + m2 + m3);
			}
		...
		getGateId() + getUseSuperPassword() + superPasswordSum
				...
				+ useFireFighting+clientTrigger+actualTimeSum+first;
