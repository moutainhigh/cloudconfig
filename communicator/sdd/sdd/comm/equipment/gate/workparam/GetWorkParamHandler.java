 ...
 * 获取门工作参数
 * */
public class GetWorkParamHandler extends
		BaseHandler<ByteBuf,GateWorkParamInterface, ReceiveData> {	
	public static final Logger LOG = Logger.getLogger(GetWorkParamHandler.class);
	/**
	 * 处理报头
	 * 
	 * @param receiveBuffer
	...
	protected GateWorkParamInterface getDataForServer(ReceiveData data) {
        ...
        if (multiOpenCardStr.length()>=1) {
        	multiOpenCardStr=multiOpenCardStr.substring(0, multiOpenCardStr.length()-1);
		}
