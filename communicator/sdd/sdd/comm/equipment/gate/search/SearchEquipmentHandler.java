 ...
 * 
 * */
public class SearchEquipmentHandler extends
BaseMultiPacketHandler<ByteBuf,EquipmentDataForServer, ReceiveData> {
	public static final Logger LOG = Logger.getLogger(SearchEquipmentHandler.class);
	/**
	 * 处理报头
	 * 
	 * @param receiveBuffer
	...
	protected String executeResponse(ByteBuf receiveBuffer) {
         ...
         * 处理有效数据
         */
		ByteBuf dataBuf = receiveBuffer.readBytes(34);// 取34字节数据
		ReceiveData data = executeData(dataBuf);
		receiver.setData(data);
		receiver.getData().setMac(mac);
		dataBuf = null;
		/*************************************************************/
		int orderStatus = receiveBuffer.readUnsignedByte();// 命令状态
		receiver.setOrder(orderStatus);
		int checkFromEquipment = receiveBuffer.readUnsignedShort();// 校验和
		receiver.setCrc(checkFromEquipment);
		
		long crcFromServer = receiver.getCrcFromSum();
		
		EquipmentDataForServer serverInfo = getDataForServer(data);
		String returnValue = null;
		if(crcFromServer==checkFromEquipment){
			LOG.info("数据接收成功，校验通过！");
		...
		LOG.info("客户端接收的十六进制信息:"
				...
				+ Util.lpad(Integer.toHexString(checkFromEquipment),2) + "|");
		EquipmentDataForServer dataForServer = getDataForServer(receiver
				.getData());
		return returnValue;
	}
	...
	protected ReceiveData executeData(ByteBuf dataBuf) {
		 ...
		 * 获取设备版本号
		 */
		short mechineVersionA = dataBuf.readUnsignedByte();
		short mechineVersionB = dataBuf.readUnsignedByte();
		data.setMechineVersionA(mechineVersionA);
		data.setMechineVersionB(mechineVersionB);
		/**
		 * 获取程序版本号 0xyymmdd：表示设备程序版本号。如1409
		 */
		short programVersionA = dataBuf.readUnsignedByte();
		short programVersionB = dataBuf.readUnsignedByte();
		short programVersionC = dataBuf.readUnsignedByte();
		data.setProgramVersionA(programVersionA);
		data.setProgramVersionB(programVersionB);
		data.setProgramVersionC(programVersionC);
		/**
		...
		receiveData.setEquipmentData(data);
		return receiveData;
	}
    /**
     * 处理发送给服务器的数据
     ...
     * @return
     */
	public EquipmentDataForServer getDataForServer(ReceiveData receiveData) {
		...
		StringBuilder bufferForMechineVersion = new StringBuilder();
		...
		String mechineEventPortHex = mechineEventPortBuffer.toString();
		...
		String remoteEventPortHex = remoteEventPortBuffer.toString();
		int remoteEventPort = Integer.valueOf(remoteEventPortHex, 16);
		returnData.setRemoteEventPort(String.valueOf(remoteEventPort));
		
		short cardSign = data.getCardSign();
		returnData.setCardSign(String.valueOf(cardSign));
       
		/**
		 * mac地址
		 */
		returnData.setMac(String.valueOf(Integer.toHexString(receiveData.getMac())));
		return returnData;
	}
