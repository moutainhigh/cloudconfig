 ...
 * 
 * */
public class GetGroupHandler extends
		BaseHandler<ByteBuf,GateLimitInterface, ReceiveData> {	
	public static final Logger LOG = Logger.getLogger(GetGroupHandler.class);	
	/**
	 * 处理报头
	 * 
	 * @param receiveBuffer
	...
	protected String executeResponse(ByteBuf receiveBuffer) {
		...
		//设置有效数据
		ByteBuf dataBuf = receiveBuffer.readBytes(513);// 取8字节有效数据
		...
		LOG.info("crcfromsum"+receiver.getCrcFromSum());
//		GateLimitInterface serverInfo = getDataForServer(data);
	...
	protected ReceiveData executeData(ByteBuf dataBuf) {
        ...
        for(int i=0;i<128;i++){
        	...
        	start.setHour(dataBuf.readUnsignedByte());
        	start.setMinute(dataBuf.readUnsignedByte());
        	end.setHour(dataBuf.readUnsignedByte());
        	end.setMinute(dataBuf.readUnsignedByte());
