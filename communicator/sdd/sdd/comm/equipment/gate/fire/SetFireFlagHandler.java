 ...
 * 设置取消消防
 * */
public class SetFireFlagHandler extends
		BaseHandler<ByteBuf,GateTimeInterface, ReceiveData> {	
	public static final Logger LOG = Logger.getLogger(SetFireFlagHandler.class);	
	/**
	 * 处理报头
	 * 
	 * @param receiveBuffer
...
//		GateTimeInterface serverInfo = getDataForServer(data);
	...
	protected ReceiveData executeData(ByteBuf dataBuf) {
		 ...
		 * 读取是否启用消防
		 */
		int isUseFire = dataBuf.readUnsignedByte();
		gateData.setUseFireFighting(isUseFire);
		LOG.info("isUseFire="+isUseFire);
