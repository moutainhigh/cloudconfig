...
public class SetTriggerHandler extends
		BaseHandler<ByteBuf,GateTimeInterface, ReceiveData> {	
	public static final Logger LOG = Logger.getLogger(SetTriggerHandler.class);
	/**
	 * 处理报头
	 * 
	 * @param receiveBuffer
...
//		GateTimeInterface serverInfo = getDataForServer(data);
	...
	protected ReceiveData executeData(ByteBuf dataBuf) {
		 ...
		 * 读取上位触发参数
		 */
		int clientTrigger = dataBuf.readUnsignedByte();
		gateData.setClientTrigger(clientTrigger);
