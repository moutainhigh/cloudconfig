...
public class ExcelExportTemplate {
	...
	public void createLongTitleRow(List<TitleRowCell> titleRowCells){
		String sheetName = "节假日表";//表头
		sheet =  (HSSFSheet) workbook.createSheet(sheetName);
		for(int i=0;i<titleRowCells.size(); i++){
			sheet.setColumnWidth(i, 6000);
		}
		sheet.setColumnWidth(5, 12000);  //作用部门需要较大列宽
		/*设置表头开始*/
        ...
        for(int i=1;i<5;i++){
        	...
        	if(i==1){
        		cell0.setCellValue("说明:1.支持时间格式：2000-01-01 (年-月-日)");
        	}else if(i==2){
        		cell0.setCellValue("          2.如果该节假日为补班类型，时段只能选择“全天” ");
        	}else if(i==3){
        		cell0.setCellValue("          3.描述（不可超过150字） ");
        	} else{  
