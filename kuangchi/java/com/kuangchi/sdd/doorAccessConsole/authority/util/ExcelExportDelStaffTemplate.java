 ...
 *
 */
public class ExcelExportDelStaffTemplate {
	...
	public ExcelExportDelStaffTemplate() {
	...
	public void createLongTitleRow(List<TitleRowCell> titleRowCells){
        ...
        for(int i=1;i<5;i++){
        	...
        	if(i==1){
        		cell0.setCellValue("说明:1.员工工号：请选择有绑定卡片的员工工号");
        	}else if(i==2){
        		cell0.setCellValue("说明:2.员工名称：只能为中文，数字，字母，空格或特殊符号,或. 且长度不能超过50位");
        	}else if(i==3){ 
        		cell0.setCellValue("          3.门名称：点开页面数据行,选中卡数据行查看权限，设备下有关联门的门名称，门可重复 ");
        	}else{
        		cell0.setCellValue("          4.浅绿色标题栏下的项目为必填项");
        	}
        }
...
private void createLongSuggest(Cell cell,List<String> sugs,int suggestIndex){
	...
	public static void main(String[] args) throws IOException {
		...
		ExcelExportDelStaffTemplate e = new ExcelExportDelStaffTemplate();
