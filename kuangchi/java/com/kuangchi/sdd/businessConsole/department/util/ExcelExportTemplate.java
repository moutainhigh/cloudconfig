package com.kuangchi.sdd.businessConsole.department.util;
...
public class ExcelExportTemplate {
	...
	public void createLongTitleRow(List<TitleRowCell> titleRowCells){
        ...
        for(int i=1;i<4;i++){
        	...
        	if(i==1){
        		cell0.setCellValue("说明:1.上级部门编号、部门编号：只能为字母或数字组合,且长度不超过20位");
        	} else if(i==2){
        		cell0.setCellValue("          2.部门名称、简称的长度不超过100位，备注的长度不超过200位");
        	}else{
	...
	private void createLongSuggest(Cell cell,List<String> sugs){
	    ...
	    CellRangeAddressList  addressList = new CellRangeAddressList(4, 65535, index, index);
