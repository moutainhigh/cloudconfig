package com.kuangchi.sdd.attendanceConsole.dutyuser.util;
...
public class ExcelExportTemplate {
	...
	public void createDeptLongTitleRow(List<TitleRowCell> titleRowCells){
		String sheetName = "部门排班表";//表头
        ...
        for(int i=1;i<4;i++){
        	...
        	CellRangeAddress region = new CellRangeAddress(i, i, 0, columnsNum-1); // 参数都是从O开始   
        	...
        	if(i==1){
        		cell0.setCellValue("说明:1.排班开始时间和排班结束时间格式如：2000-01-02 (年-月-日)");
        	}else if(i==2){
        		cell0.setCellValue("          2.排班开始时间不能晚于排班结束时间，排班开始时间不能早于当天日期 ");
        	}else{ 
		...
		/*	if(i==0){
		        ...
		        createLongTitleRowCell(titleRowCells.get(i),cell,0);
			}else{			//不超过255时调用的方法
				CellStyle titleCellStyle1=workbook.createCellStyle();
	...
	public void createLongTitleRow(List<TitleRowCell> titleRowCells){
        ...
        for(int i=1;i<4;i++){
        	HSSFRow row1 = sheet.createRow(i);
        	Cell cell0 = row1.createCell(0);
        	CellRangeAddress region = new CellRangeAddress(i, i, 0, 6); // 参数都是从O开始   
            sheet.addMergedRegion(region); 
            CellStyle titleCellStyle0=workbook.createCellStyle();
            titleCellStyle0.setFont(titleFont0);
            titleCellStyle0.setWrapText(true);
	...
	private void createTitleRowCell(TitleRowCell t,Cell cell){
		...
		if(t.getSuggest().size() > 0){
			createSuggest(cell,t.getSuggest());        //设置整列为下拉框
		}else{
			sheet.setDefaultColumnStyle(column,style); //设置整列为文本形式
		}
	}
	/**
	 * 创建列
