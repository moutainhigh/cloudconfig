...
import com.sun.xml.ws.client.sei.ResponseBuilder.Composite;
...
public class CalibrationSpiderWebPlot extends SpiderWebPlot {
	...
	public static final int DEFAULT_TICKS = 10; // 圈数
	public static final double DEFAULT_MAX_VALUE = 10; // 坐标最大值
	...
	protected void drawLabel(final Graphics2D g2, final Rectangle2D plotArea, final double value, final int cat,
		...
		for (int i = 1; i <= ticks; i++) {
			...
			rotateTrans.transform(transformer, 0, transformed, 0, 1);
			final java.awt.Composite saveComposite = g2.getComposite();
			...
			if (dataValue != null) {
				...
				if (value >= 0) { // draw the polygon series...
					...
					if (entities != null) {
						...
						Rectangle area = new Rectangle((int) (point.getX() - headW), (int) (point.getY() - headH),
