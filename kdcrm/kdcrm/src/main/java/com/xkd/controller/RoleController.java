...
//            }
    ...
    @ApiOperation(value ="转移角色")
    @ResponseBody
    @RequestMapping(value = "/transferRole", method = {RequestMethod.GET, RequestMethod.POST})
    public ResponseDbCenter updateRole(HttpServletRequest req, HttpServletResponse rsp,
                                       @ApiParam(value = "角色Id",required = true) @RequestParam(required = true) String roleId,
                                       @ApiParam(value = "角色名称",required = true)@RequestParam(required = true) String userIds)
            throws Exception {
        try {
            roleService.updateUserRole(new ArrayList(Arrays.asList((userIds.trim()).split(","))),roleId);
        } catch (Exception e) {
            e.printStackTrace();
            throw new GlobalException(ResponseConstants.FUNC_SERVERERROR);
        }
