...
public class CustomerController {
    ...
    public ResponseDbCenter searchPcCompany(HttpServletRequest req, HttpServletResponse rsp,
        ...
        try {
            ...
            Integer count=0;
            mapList = customerService.searchPcCompany(startDate, endDate, departmentName, province, city, status,null, currentPage, pageSize);
            count= customerService.searchPcCompanyCount(startDate, endDate, departmentName, province, city, status,null);
    ...
    public ResponseDbCenter updatePcCompany(HttpServletRequest req, HttpServletResponse rsp,
                                            ...
                                            @ApiParam(value = "联系人", required = false) @RequestParam(required = false) String  contactor,
                                            ...
                                            @ApiParam(value = "资质", required = false) @RequestParam(required = false) String enterpriseLevel,
                                            @ApiParam(value = "营业执照", required = false) @RequestParam(required = false) String businessLicense,
                                            @ApiParam(value = "荣誉证书图片列表，多个以逗号分隔", required = false) @RequestParam(required = false) String honorImgList
        ...
        try {
            ...
            Map<String,Object> map=new HashMap<>();
                ...
                map.put("enterpriseLevel",enterpriseLevel);
                map.put("businessLicense",businessLicense);

               if (StringUtils.isNotBlank(honorImgList)){
                   String[] urls=honorImgList.split(",");
                   ...
                   attachmentService.insertAttachmentList(honorList);
               }
    ...
    @ApiOperation(value = "查询某一个服务商下面的管理员")
    @ResponseBody
    ...
    public ResponseDbCenter changePcUser(HttpServletRequest req,HttpServletResponse rsp,
        ...
        map.put("roleId","2");//角色为服务商
        try {

            if("insert".equals(flag)){
                String insertUserId=UUID.randomUUID().toString().replace("-", "");
                map.put("id", insertUserId);  //去掉横线，以便极光可以进行消息推送
                map.put("createDate",new Date());
                ...
                String enPassWord = new String(EncryptUtil.encryptPassword(password));
                 ...
                 */
//                //添加服务商下设备告警名单
//                apiCallFacadeService.addDeviceWarningWhenBindServerAccount(pcCompanyId,(String)map.get("id"));


                //同步用户
                apiCallFacadeService.registerUserCallFacede(mobile,password,"2",insertUserId,mobile,email);
...
//                apiCallFacadeService.addDeviceWarningWhenBindServerAccount(pcCompanyId,(String)map.get("id"));
                ...
                //同步用户
                apiCallFacadeService.updateUserCallFacede(mobile,null,"2",id,mobile,mobile,email,null,null,null,null);
